...
- **Pin / unpin** — `pin(id)` / `unpin(id)` on the feeds namespace in all four SDKs (`write:statuses`).

### Changed
- **`get_following` / `getFollowing` / `GetFollowing` now take a `surf_id` first argument.** The endpoint is a reverse lookup (public custom feeds that include the given Surf ID as a source), not the caller's subscriptions, and always required `surf_id` server-side, so the old zero-argument form returned a 400. The caller's own pinned feeds are `feedPins` in `preferences.get()`. Returns a bare list.
- **`services` on feed posts** — the canonical value is a list of service ids (`["surf/service/bluesky", "surf/service/mastodon"]`, sent as repeated params); the Python and TypeScript SDKs now accept a list as well as a string, and the server also accepts bare names (`"bluesky,rss"`), which previously failed with an unrelated `surfId` error. When omitted the API now defaults to Mastodon, RSS, Patreon **and Bluesky** for API-key callers (Bluesky used to be opt-in on topic/hashtag/custom feeds, and `bluesky/customfeed/*` feeds returned `[]`).
- **Create-post docs** — `visibility` is honoured on Mastodon only; a non-public value on a Bluesky target is now rejected with 400 instead of publishing publicly. `service` is now honoured on every write (it was silently ignored, so `service=mastodon` posted to Bluesky and Mastodon interactions failed with `Invalid AT URI`); a numeric Mastodon status id in the path infers `mastodon` automatically, but replying to a Mastodon post still needs `service=mastodon`.

- **Podcast popularity charts** (Python sync+async, TypeScript, Go, Java) — the audio namespace wraps the two daily popularity-snapshot endpoints (`read:audio` scope). Both return rows in rank order; `limit` defaults to 50 (max 200) and `date` requests an explicit `YYYY-MM-DD` snapshot (omit for the latest, which is cached ~5 minutes server-side); the response envelope's `snapshot_date` says which daily snapshot was served (`null` when none exists yet).
  - **Popular shows** (`GET /audio/popular/shows`) — ranked popular podcast shows per region/category, blending Apple top charts, Podcast Index trending, and Surf's fediverse engagement signal; rows carry the per-source ranks (`apple_rank`, `pi_trend_rank`) for "№ 3 on Apple" style attribution plus `flyf_id`/`feed_url` for feeding the other audio APIs. `ingested_only` (wire param `ingestedOnly`; default true) limits the chart to shows already ingested and playable on Surf — pass false for the full chart (gap analysis). `client.audio.get_popular_shows(region=…, category=…, limit=…, ingested_only=…, date=…)` (Python); `client.audio.getPopularShows({ region?, category?, limit?, ingested_only?, date? })` (TypeScript, typed `PopularShowsResponse`); `client.Audio.GetPopularShows(region, category, limit, ingestedOnly, date)` (Go — `""`/`<= 0` omit an optional param); `client.audio.getPopularShows([region, category, limit, ingestedOnly, date])` (Java).
  - **Hot episodes** (`GET /audio/popular/episodes`) — the freshest episodes from the day's top-ranked shows (max 3 per show, never two from the same show adjacent), engagement-aware when that signal exists — a per-episode chart neither Apple nor Podcast Index has. Rows carry `episode_url` (the audio file URL, ready for the per-episode audio APIs), `episode_url_hash`, `show_title`, `engagement_sum`, and `post_count`. `get_popular_episodes(limit=…, date=…)` / `getPopularEpisodes({ limit?, date? })` (typed `PopularEpisodesResponse`) / `GetPopularEpisodes(limit, date)` / `getPopularEpisodes([limit, date])`.
  - **Typed models** — Python dataclasses `PopularShow` and `PopularEpisode` (each with `from_dict`/`from_list`, exported from `surf_api`); TypeScript interfaces for the same set plus the two response types; Go structs (`PopularShow`/`PopularShowsResponse`, `PopularEpisode`/`PopularEpisodesResponse`) as `json.RawMessage` decode targets. The Java SDK stays raw `Map<String, Object>`, matching its audio namespace.
- **Graded content-safety verdict on posts** (Python, TypeScript, Go) — posts returned by feed and search endpoints now carry a `safety` object: an ordinal `rating` (`explicit` | `suggestive` | `safe` | `unknown`), the verbatim Bluesky `labels` behind it (`porn`, `sexual`, `nudity`, `graphic-media`, `sexual-figurative`, `bot`, and any future labeler value — the vocabulary is open, unrecognized values are carried rather than dropped), and a `source` for provenance (`self-label` | `bsky-moderation` | `flipboard-detection` | `none`). Prefer it over the `sensitive` boolean: `unknown` (no trusted signal — the default) is a distinct tier from `safe` (affirmatively cleared), a distinction the boolean cannot express. `safety=sfw` on the request now filters on the graded verdict, dropping `explicit` and `suggestive` server-side while keeping `unknown`, which for un-labelled sources (RSS, cold Mastodon) is most of the pool — apply a stricter "drop unknown too" policy client-side using these fields. Surfaced as `Post.safety` (Python `PostSafety` dataclass, exported from `surf_api`; TypeScript `PostSafety` interface; Go `*PostSafety`); the Java SDK returns raw maps for posts and is unaffected.
